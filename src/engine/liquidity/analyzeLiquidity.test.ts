...
const swing = (index: number, price: number, type: 'high' | 'low'): SwingPoint => ({
  index, price, time: index, type,
})
...
describe('Liquidity Engine', () => {
  ...
  it('detects equal lows as sell-side liquidity', () => {
    const result = analyzeLiquidity(baseCandles(), {
      highs: [],
      lows: [swing(1, 8.9995, 'low'), swing(3, 8.9998, 'low')],
    }, 0.001)
  ...
  it('clusters nearby same-side swings and assigns strength by touches', () => {
    const result = analyzeLiquidity(baseCandles(), {
      highs: [
        swing(1, 11.0001, 'high'),
        swing(2, 11.0002, 'high'),
        swing(3, 11.0003, 'high'),
        swing(4, 11.0004, 'high'),
      ],
      lows: [],
    }, 0.001)
  ...
  it('keeps clearly separated pools separate', () => {
    const result = analyzeLiquidity(baseCandles(), {
      highs: [swing(1, 11, 'high'), swing(3, 13, 'high')],
      lows: [],
    }, 0.001)
  ...
  it('finds the nearest unswept buy-side target above price', () => {
    const result = analyzeLiquidity(baseCandles(10.5), {
      highs: [swing(1, 11.5, 'high'), swing(2, 12, 'high')],
      lows: [],
    }, 0.001)
  ...
  it('finds the nearest unswept sell-side target below price', () => {
    const result = analyzeLiquidity(baseCandles(10.5), {
      highs: [],
      lows: [swing(1, 8.5, 'low'), swing(2, 8, 'low')],
    }, 0.001)

    expect(result.nearestSellSide?.referencePrice).toBe(8.5)
  })

  it('detects a buy-side sweep using a wick through the level and close back below', () => {
    const candles = [...baseCandles(10), candle(6, 10, 12, 9.5, 10.5)]
    const result = analyzeLiquidity(candles, { highs: [swing(1, 11, 'high')], lows: [] }, 0.001)

    expect(result.pools[0].isSwept).toBe(true)
    expect(result.nearestBuySide).toBeNull()
  })

  it('does not call a clean close above a level a liquidity sweep', () => {
  ...
  it('detects a sell-side sweep using a wick through the level and close back above', () => {
    const candles = [...baseCandles(10), candle(6, 9.5, 8, 7.5, 9)]
    const result = analyzeLiquidity(candles, { highs: [], lows: [swing(1, 8.5, 'low')] }, 0.001)
  ...
  it('ignores malformed candles safely', () => {
    const malformed = [candle(1, 10, 9, 8, 10)]
    const result = analyzeLiquidity(malformed, { highs: [swing(1, 11, 'high')], lows: [] }, 0.001)
  ...
  it('is deterministic', () => {
    ...
    const first = analyzeLiquidity(baseCandles(10), input, 0.001)
    const second = analyzeLiquidity(baseCandles(10), input, 0.001)
    expect(first).toEqual(second)
  })

  it('keeps buy-side and sell-side pools conceptually separate', () => {
    const result = analyzeLiquidity(baseCandles(), {
      highs: [swing(1, 11, 'high')],
      lows: [swing(2, 9, 'low')],
    }, 0.001)
