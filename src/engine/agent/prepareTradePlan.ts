...
export const prepareTradePlan = (input: TradePlanInput): TradePlan => {
  ...
  if (!risk.isValid) return { isValid: false, setup: input.setup, risk, lotSize: 0, estimatedLoss: 0, estimatedReward: 0, summary: risk.errorMessage ?? 'The trade plan failed risk validation.' }
  const estimatedReward = Number((risk.estimatedLossAtStop * risk.riskRewardRatio).toFixed(2))
  return { isValid: true, setup: input.setup, risk, lotSize: risk.suggestedLotSize, estimatedLoss: risk.estimatedLossAtStop, estimatedReward, summary: `${input.setup.direction} plan validated at ${risk.suggestedLotSize.toFixed(2)} lots with ${risk.riskRewardRatio.toFixed(2)}R reward potential.` }
}
