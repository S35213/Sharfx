    ...
    return <main className="min-h-[calc(100vh-28px)] overflow-y-auto bg-shafx-bg text-shafx-text">
      <div className="mx-auto flex min-h-[calc(100vh-28px)] w-full max-w-[1480px] items-center justify-center px-4 py-8 sm:px-8">
        <div className="w-full max-w-md">
          <button type="button" onClick={() => { setShowAuth(false); setForgotMode(false); setMessage(null) }} className="mb-6 flex items-center gap-2 text-[11px] text-shafx-textMuted hover:text-shafx-text"><ArrowRight className="h-3.5 w-3.5 rotate-180" />Back to SHAFX welcome</button>
          <div className="mb-7 flex items-center gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-xl border border-shafx-accent/30 bg-shafx-accent/10 text-shafx-accent"><Activity className="h-5 w-5" /></div><div><div className="font-semibold tracking-tight">SHAFX</div><div className="text-[9px] uppercase tracking-[0.24em] text-shafx-textMuted">Market workspace</div></div></div>
          ...
          <div className="mt-8 flex items-start gap-2 border-t border-shafx-border pt-5 text-[10px] leading-5 text-shafx-textMuted"><LockKeyhole className="mt-0.5 h-3.5 w-3.5 flex-shrink-0 text-shafx-accent" /><span>SHAFX identity is separate from connected provider credentials. Provider secrets remain behind the server-side boundary.</span></div>
        </div>
      </div>
    </main>
  }
