...
export function CandlestickChart({ candles, annotations = [] }: CandlestickChartProps) {
  ...
  const safeCandles = useMemo(() => {
    ...
    return sorted.filter((c) => { const time = Number(c.time); if (seen.has(time)) return false; seen.add(time); return c.high >= Math.max(c.open, c.close) && c.low <= Math.min(c.open, c.close) })
  }, [candles])
  useEffect(() => {
