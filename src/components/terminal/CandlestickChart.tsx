...
export function CandlestickChart({ candles, annotations = [] }: CandlestickChartProps) {
  ...
  useEffect(() => {
    ...
    const chart = createChart(containerRef.current, {
      autoSize: true,
      layout: { background: { color: '#0B0E11' }, textColor: '#9AA4B2' },
      grid: { vertLines: { color: '#1B222C' }, horzLines: { color: '#1B222C' } },
      rightPriceScale: { borderColor: '#29313D' },
      timeScale: { borderColor: '#29313D', timeVisible: true },
      crosshair: { mode: 0 },
    })
  ...
  useEffect(() => {
    const series = seriesRef.current
    if (!series) return
    const created = annotations.filter((a) => Number.isFinite(a.price)).map((a) => series.createPriceLine({
      price: a.price,
      color: a.type === 'stop' ? '#ef5350' : a.type === 'target' ? '#26a69a' : a.type === 'entry' ? '#2962FF' : '#9AA4B2',
      lineWidth: a.type === 'entry' ? 2 : 1,
      lineStyle: a.type === 'liquidity' ? 2 : 0,
