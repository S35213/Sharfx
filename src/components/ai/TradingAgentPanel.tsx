...
export function TradingAgentPanel({ symbol, timeframe, candles, currentPrice, tradeHistory, accountBalance = 10000, onReviewSetup }: Props) {
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      ...
      if (!setup) { setLastResult('WAIT'); setStatus('No clean setup — bot is waiting'); return }
      const win = Math.random() >= 0.4
      if (win) { setWins((value) => value + 1); setLastResult('WIN'); setStatus('Simulated profit — scanning again') }
      else {
        setLosses((value) => {
          const next = value + 1
          if (next >= 2) { setRunning(false); setStatus('Stopped after 2 simulated losses — review strategy') }
          return next
        })
        setLastResult('LOSS')
