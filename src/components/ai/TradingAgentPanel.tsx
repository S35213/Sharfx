...
import { Activity, Bot, CheckCircle2, ChevronDown, CircleStop, Play, ShieldCheck, Sparkles, Wallet } from 'lucide-react'
...
export function TradingAgentPanel({ symbol, timeframe, candles, currentPrice, activePosition, tradeHistory, accountBalance = 10000, onReviewSetup }: Props) {
  ...
  const direction = setup?.direction ?? (tradingContext.structureBias === 'Bullish' ? 'BUY' : tradingContext.structureBias === 'Bearish' ? 'SELL' : null)
  ...
  return <div className="space-y-3 rounded-lg border border-shafx-border bg-shafx-surface p-3 text-sm shadow-sm">
    <div className="flex items-start justify-between gap-3">
      <div>
        <h3 className="flex items-center gap-2 font-semibold"><Bot className="h-4 w-4 text-shafx-primary" />SHAFX Bot</h3>
        <p className="mt-1 text-[11px] leading-relaxed text-shafx-textMuted">A simple paper-trading assistant: analyze → choose risk → simulate → learn.</p>
      </div>
      <span className={`flex items-center gap-1 rounded border px-2 py-1 text-[10px] ${running ? 'border-shafx-success/20 bg-shafx-success/10 text-shafx-success' : 'border-shafx-border text-shafx-textMuted'}`}><Activity className="h-3 w-3" />{running ? 'Running' : 'Ready'}</span>
    </div>

    <div className="rounded-md border border-shafx-border bg-shafx-bg p-3">
      <div className="mb-2 text-[10px] uppercase tracking-wider text-shafx-textMuted">1. Analyze the market</div>
      ...
      <p className="mt-2 text-xs leading-relaxed text-shafx-textMuted">{setup ? `The bot found a ${setup.direction} setup around ${setup.entryPrice}. It checks stop, target and risk before a simulated trade.` : 'The bot does not see a clean setup yet, so it waits instead of forcing a trade.'}</p>
    </div>

    <div className="rounded-md border border-shafx-primary/30 bg-shafx-primary/5 p-3">
      <div className="mb-2 flex items-center gap-2"><ShieldCheck className="h-4 w-4 text-shafx-primary" /><div><div className="text-[10px] uppercase tracking-wider text-shafx-textMuted">2. Choose risk</div><div className="font-semibold">{riskModes[riskMode].label} mode</div></div></div>
      ...
      <div className="mt-2 flex items-center justify-between text-xs"><span className="text-shafx-textMuted">Demo balance</span><span className="font-mono">${accountBalance.toFixed(2)}</span></div>
      <div className="flex items-center justify-between text-xs"><span className="text-shafx-textMuted">Max risk per simulated trade</span><span className="font-mono text-shafx-danger">${riskAmount.toFixed(2)}</span></div>
    </div>

    <div className="rounded-md border border-shafx-border bg-shafx-bg p-3">
      <div className="mb-2 flex items-center justify-between"><div><div className="text-[10px] uppercase tracking-wider text-shafx-textMuted">3. Trade with bot</div><div className="font-semibold">{cycleSeconds}-second scan cycle</div></div><Wallet className="h-4 w-4 text-shafx-primary" /></div>
      <div className="grid grid-cols-2 gap-2">{([5, 10] as const).map((seconds) => <button key={seconds} type="button" onClick={() => setCycleSeconds(seconds)} className={`min-h-10 rounded border text-xs ${cycleSeconds === seconds ? 'border-shafx-primary bg-shafx-primary/10 text-shafx-primary' : 'border-shafx-border text-shafx-textMuted'}`}>{seconds}s cycle</button>)}</div>
      <div className="mt-2 grid grid-cols-2 gap-2"><button type="button" disabled={running || !setup} onClick={startBot} className="flex min-h-11 items-center justify-center gap-2 rounded bg-shafx-success px-3 py-2 text-xs font-semibold text-white disabled:cursor-not-allowed disabled:opacity-40"><Play className="h-4 w-4" />Start bot</button><button type="button" disabled={!running} onClick={stopBot} className="flex min-h-11 items-center justify-center gap-2 rounded border border-shafx-danger/30 text-xs font-semibold text-shafx-danger disabled:opacity-40"><CircleStop className="h-4 w-4" />Stop</button></div>
      <div className="mt-3 rounded border border-shafx-border p-2 text-xs"><div className="font-medium">{status}</div><div className="mt-1 flex justify-between text-[10px] text-shafx-textMuted"><span>Cycles {cycles}</span><span>Wins {wins} • Losses {losses}/2</span></div>{lastResult && <div className={`mt-1 text-[10px] ${lastResult === 'WIN' ? 'text-shafx-success' : lastResult === 'LOSS' ? 'text-shafx-danger' : 'text-shafx-textMuted'}`}>{lastResult === 'WIN' ? 'Profit → start next scan' : lastResult === 'LOSS' ? 'Loss → check setup before continuing' : 'Waiting for a setup'}</div>}</div>
    </div>

    <div className="rounded-md border border-shafx-border bg-shafx-bg p-3">
      <div className="text-[10px] uppercase tracking-wider text-shafx-textMuted">How the bot reacts</div>
      <div className="mt-2 grid gap-2 text-xs text-shafx-textMuted"><div><span className="font-semibold text-shafx-text">1.</span> Scan the selected market.</div><div><span className="font-semibold text-shafx-text">2.</span> If a setup passes the risk rules, run one simulated trade.</div><div><span className="font-semibold text-shafx-text">3.</span> After a simulated profit, start the next scan automatically.</div><div><span className="font-semibold text-shafx-text">4.</span> After two simulated losses, stop and show a strategy-review message.</div></div>
    </div>

    <div className="rounded-md border border-shafx-border bg-shafx-bg p-3">
      <div className="flex items-center justify-between gap-3"><div><div className="text-[10px] uppercase tracking-wider text-shafx-textMuted">Manual trade</div><div className="font-semibold">Trade yourself</div><p className="mt-1 text-[11px] text-shafx-textMuted">Manual orders stay in the Market screen so the chart, price and order ticket remain together.</p></div><Sparkles className="h-4 w-4 text-shafx-primary" /></div>
      <div className="mt-2 grid grid-cols-2 gap-2"><button type="button" onClick={onReviewSetup} className="min-h-11 rounded bg-shafx-success px-3 text-xs font-semibold text-white">BUY {symbol}</button><button type="button" onClick={onReviewSetup} className="min-h-11 rounded bg-shafx-danger px-3 text-xs font-semibold text-white">SELL {symbol}</button></div>
    </div>
