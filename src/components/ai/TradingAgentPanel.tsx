...
export function TradingAgentPanel({
  ...
  return (
    <>
      <section className="rounded-2xl border border-shafx-border bg-shafx-surface p-3.5 text-sm shadow-[0_14px_36px_rgba(0,0,0,.18)] sm:p-4">
        ...
        <div className="mt-3 rounded-xl border border-shafx-border bg-shafx-bg p-3.5">
          <div className="flex items-center justify-between gap-3">
            <div><div className="text-[9px] font-semibold uppercase tracking-[0.14em] text-shafx-textMuted">Trade size</div><div className="mt-1 text-sm font-semibold">Lot size</div></div>
        ...
        {!botDisplayedOrder && autoTradingEnabled && (
          <div className="mt-3 rounded-xl border border-shafx-accent/20 bg-shafx-accent/[0.035] p-3">
            <div className="flex items-center justify-between gap-2"><span className="text-xs font-semibold text-shafx-text">{phase === 'ANALYZING' ? 'Checking the market…' : 'Watching for the next trade…'}</span><span className="font-mono text-[9px] text-shafx-accent">{symbol}</span></div>
      ...
      <div className="hidden" aria-hidden="true">{oldReturnExpr}</div>
    </>
  )
}
