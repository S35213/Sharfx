...
import { Brain, CheckCircle2, Shield, Eye, Activity, History } from 'lucide-react'
...
import { decideAgentAction, analyzeMultiTimeframeBias, monitorPosition, learnFromTrades } from '../../engine/agent'
...
export function TradingAgentPanel({ symbol, timeframe, candles, currentPrice, activePosition, tradeHistory, onReviewSetup }: Props) {
  ...
  return <div className="space-y-3 rounded-lg border border-shafx-primary/30 bg-shafx-surface p-4 text-sm">
    <div className="flex items-start justify-between gap-2"><div><h3 className="flex items-center gap-2 font-semibold"><Brain className="h-4 w-4 text-shafx-primary" /> Agent control</h3><p className="mt-1 text-[11px] text-shafx-textMuted">The agent reasons across the selected chart, higher timeframes and recorded simulator outcomes while keeping execution behind your approval.</p></div><Shield className="h-4 w-4 text-shafx-primary" /></div>
