    ...
    <section className="rounded-2xl border border-shafx-border bg-shafx-surface p-3.5 text-sm shadow-[0_14px_36px_rgba(0,0,0,.22)] sm:p-4">
      ...
      <section className="mt-3 rounded-xl border border-shafx-border bg-shafx-bg p-3.5">
        <div className="flex items-center justify-between gap-3">
          <div>
            <div className="text-[9px] font-semibold uppercase tracking-[0.14em] text-shafx-textMuted">Lot size</div>
            <div className="mt-1 text-sm font-semibold">Trade amount</div>
          </div>
        ...
        {botDisplayedOrder && (
          <div className="mt-3 rounded-xl border border-shafx-success/25 bg-shafx-success/[0.035] p-3">
            <div className="flex items-center justify-between gap-2">
              <div className={botDisplayedOrder.type === 'BUY' ? 'text-sm font-bold text-shafx-success' : 'text-sm font-bold text-shafx-danger'}>{botDisplayedOrder.type} {botDisplayedOrder.symbol}</div>
              <span className="font-mono text-[9px] text-shafx-textMuted">{Math.max(0, tradeSecondsLeft).toFixed(1)}s</span>
            </div>
            <div className="mt-2 grid grid-cols-3 gap-2 text-[9px]">
              <div className="rounded-lg border border-shafx-border bg-shafx-surface px-2 py-2"><span className="block text-shafx-textMuted">Entry</span><b className="font-mono">{botDisplayedOrder.entryPrice.toFixed(symbolSpec?.pricePrecision ?? 5)}</b></div>
              <div className="rounded-lg border border-shafx-danger/20 bg-shafx-surface px-2 py-2"><span className="block text-shafx-textMuted">Stop</span><b className="font-mono text-shafx-danger">{botDisplayedOrder.stopLoss?.toFixed(symbolSpec?.pricePrecision ?? 5) ?? '—'}</b></div>
              <div className="rounded-lg border border-shafx-success/20 bg-shafx-surface px-2 py-2"><span className="block text-shafx-textMuted">Target</span><b className="font-mono text-shafx-success">{botDisplayedOrder.takeProfit?.toFixed(symbolSpec?.pricePrecision ?? 5) ?? '—'}</b></div>
            </div>
          </div>
        )}
        ...
        <div className="mt-3 grid grid-cols-3 gap-2 text-[9px]">
          <div className="rounded-xl border border-shafx-border bg-shafx-surface px-2.5 py-2.5"><span className="block text-shafx-textMuted">Won</span><strong className="mt-0.5 block font-mono text-shafx-success">{wins}</strong></div>
          <div className="rounded-xl border border-shafx-border bg-shafx-surface px-2.5 py-2.5"><span className="block text-shafx-textMuted">Lost</span><strong className="mt-0.5 block font-mono text-shafx-danger">{losses}</strong></div>
          <div className="rounded-xl border border-shafx-border bg-shafx-surface px-2.5 py-2.5"><span className="block text-shafx-textMuted">Units</span><strong className="mt-0.5 block font-mono">{cycleUnits}/{plan.maxDailyCycleUnits === null ? '∞' : plan.maxDailyCycleUnits}</strong></div>
        </div>
        ...
        <div className="mt-2 flex items-center justify-between text-[9px] text-shafx-textMuted"><span>{autoTradingEnabled ? (phase === 'ANALYZING' ? 'Checking the market…' : 'Bot is running') : status}</span><span className="font-mono">{symbol}</span></div>
      </section>

      <section className="hidden mt-3 rounded-xl border border-shafx-border bg-shafx-bg p-3.5">
        ...
        <p className="mt-2 text-[9px] text-shafx-textMuted">Your lot size is used for every simulator round. Five rounds make one unit. Free Bot simulator testing is currently unlimited.</p>
      </section>

      {showBotActivity && (
        <section className="hidden mt-3 rounded-xl border border-shafx-accent/25 bg-shafx-accent/[0.04] p-3.5">
            ...
            <div className="mt-3 rounded-xl border border-shafx-success/30 bg-shafx-success/5 p-3">
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-2 grid grid-cols-3 gap-2 text-[9px]"><div className="rounded-lg border border-shafx-border bg-shafx-bg p-2"><span className="block text-shafx-textMuted">Entry</span><b className="font-mono">{botDisplayedOrder.entryPrice.toFixed(symbolSpec?.pricePrecision ?? 5)}</b></div><div className="rounded-lg border border-shafx-danger/20 bg-shafx-danger/[0.04] p-2"><span className="block text-shafx-textMuted">Stop Loss</span><b className="font-mono text-shafx-danger">{botDisplayedOrder.stopLoss?.toFixed(symbolSpec?.pricePrecision ?? 5) ?? '—'}</b></div><div className="rounded-lg border border-shafx-success/20 bg-shafx-success/[0.04] p-2"><span className="block text-shafx-textMuted">Take Profit</span><b className="font-mono text-shafx-success">{botDisplayedOrder.takeProfit?.toFixed(symbolSpec?.pricePrecision ?? 5) ?? '—'}</b></div></div>
      ...
      <section className="hidden relative mt-3 overflow-hidden rounded-2xl border border-shafx-accent/25 bg-[radial-gradient(circle_at_top_right,rgba(124,92,252,.20),transparent_42%),linear-gradient(145deg,#0D121A_0%,#090D14_100%)] p-3.5 shadow-[0_12px_36px_rgba(124,92,252,.10)]">
      ...
      <section className="hidden relative mt-3 overflow-hidden rounded-2xl border border-shafx-accent/20 bg-[linear-gradient(160deg,#0D121A_0%,#080C12_100%)] p-3.5">
      ...
      <section className="hidden mt-3 grid gap-3 sm:grid-cols-2">
      ...
      <section className="hidden mt-3 rounded-xl border border-shafx-border bg-shafx-bg p-3"><div className="flex items-center justify-between gap-3"><span className="text-[9px] font-semibold uppercase tracking-[0.15em] text-shafx-textMuted">Current status</span><span className="text-[9px] text-shafx-textMuted">Round {unitRound}/5</span></div><p className="mt-1.5 text-[11px] text-shafx-text">{status}</p><p className="mt-1 text-[9px] text-shafx-textMuted">Manual Scan = 10 seconds with lower-timeframe priority. Run shows a short scan phase, then a 10-second active round, shows WIN/LOSS, and starts the next round automatically.</p></section>

      <button type="button" onClick={() => setDetailsOpen((open) => !open) className="hidden} className="mt-3 flex min-h-12 w-full items-center justify-between rounded-xl border border-shafx-border bg-shafx-bg px-3 text-xs text-shafx-textMuted"><span>Advanced analysis</span><ChevronDown className={detailsOpen ? 'h-4 w-4 rotate-180 transition-transform' : 'h-4 w-4'} /></button>
