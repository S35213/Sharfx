      ...
      {items.map(({ id, label, icon: Icon }) => {
        ...
        return (
          <button
            ...
            onClick={() => onChange(id)}
            className={`flex min-h-14 min-w-16 flex-1 flex-col items-center justify-center gap-1 px-2 text-xs transition ${active ? 'text-shafx-primary' : 'text-shafx-textMuted hover:text-shafx-text'}`}
          >
