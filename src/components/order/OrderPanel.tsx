...
export const OrderPanel: React.FC<Props> = ({ symbol, currentPrice, bidPrice = currentPrice, askPrice = currentPrice, accountBalance, accountCurrency, symbolSpec, conversionRate, onSubmitOrder, aiSetup, autoApplyAISetup = false }) => {
  ...
  const marginLotCeiling = useMemo(() => {
    if (!Number.isFinite(accountBalance) || accountBalance <= 0) return 0
    let exposurePerLot = symbolSpec.contractSize
    if (symbolSpec.quoteCurrency === accountCurrency) exposurePerLot = symbolSpec.contractSize * numEntry
      ...
      exposurePerLot = symbolSpec.contractSize * numEntry * conversionRate
    }
    return Number((accountBalance * SIMULATOR_LEVERAGE / exposurePerLot).toFixed(8))
  }, [accountBalance, accountCurrency, conversionRate, numEntry, symbolSpec])
  const lotFitsAccountMargin = marginLotCeiling > 0 && parsedLotSize <= marginLotCeiling + 1e-8
