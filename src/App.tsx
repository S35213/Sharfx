...
const TerminalContent: React.FC = () => {
  ...
  const accountScopeKey = useMemo(() => {
    if (brokerMode) {
      return activeProviderSelection
    ...
    const simulatorAccountId = typeof window !== 'undefined' ? window.sessionStorage.getItem('shafx-simulator-account-id') || 'default' : 'default'
    return 'simulator:' + simulatorAccountId
  ...
  useEffect(() => {
    ...
    const load = async (): Promise<void> => {
      try {
        ...
        simulatorInitialized.current = true
        setAccountData(acc)
  ...
  }, [accountScopeKey, pushToast, selectedSymbol, timeframe])
  ...
  const reviewScannerOpportunity = useCallback((nextTimeframe: import('./types').Timeframe, nextSetup: SetupCandidate | null): void => {
    setTimeframe(nextTimeframe)
    reviewAISetup(nextSetup)
  }, [reviewAISetup, setTimeframe])
  ...
  useEffect(() => {
    ...
    const refreshPositions = async (): Promise<void> => {
      ...
      try {
        ...
        const specs = await Promise.all(uniqueSymbols.map(async (symbol) => {
          if (symbol === selectedSymbolForPositionsRef.current && symbolSpecForPositionsRef.current) return [symbol, symbolSpecForPositionsRef.current] as const
          if (symbolSpecCache.current[symbol]) return [symbol, symbolSpecCache.current[symbol]] as const
          const spec = await marketDataSource.getSymbolSpec(symbol)
          symbolSpecCache.current[symbol] = spec
          return [symbol, spec] as const
  ...
  const dockContent = {
    ...
    agent: isCompactViewport ? null : <div className="space-y-3"><TradingAgentPanel {...botProps} /><SimulationFlowChart key={selectedSymbol} selectedSymbol={selectedSymbol} openPositions={openPositions} tradeHistory={tradeHistory} /><PerformancePanel tradeHistory={tradeHistory} currency={accountData.currency} /></div>,
  ...
  return <div className={`shafx-terminal-root min-h-[100svh] w-full min-w-0 overflow-x-hidden bg-shafx-bg text-shafx-text lg:flex lg:h-[calc(100vh-28px)] lg:flex-col lg:overflow-hidden ${isLandscapeCompactViewport ? "shafx-landscape-mode" : ""}`}>
    ...
    <main className="shafx-mobile-content flex min-h-0 w-full min-w-0 flex-1 flex-col overflow-visible lg:flex-row lg:overflow-hidden">
      ...
      <aside className={`${showAgent && isCompactViewport ? '' : 'hidden'} w-full flex-shrink-0 overflow-visible p-3 pb-4 lg:hidden`}>{isCompactViewport && <div className="space-y-3"><TradingAgentPanel {...botProps} /><SimulationFlowChart key={selectedSymbol} selectedSymbol={selectedSymbol} openPositions={openPositions} tradeHistory={tradeHistory} />{brokerMode && activeProviderId === 'deriv' && <div><DerivCashierLinks /></div>}</div>}</aside>
