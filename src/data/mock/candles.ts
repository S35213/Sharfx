...
export const generateMockCandles = (opts: GenerateOptions): OHLCV[] => {
  ...
  const candles: OHLCV[] = []
  const volatility = startPrice > 100 ? 5 : 1
  for (let i = 0; i < count; i += 1) {
    const time = endSec - (count - i) * interval
    const drift = (random() - 0.48) * pipSize * 8 * volatility
    const open = price
    const close = Math.max(pipSize, open + drift)
    const high = Math.max(open, close) + random() * pipSize * 5 * volatility
    const low = Math.max(pipSize / 10, Math.min(open, close) - random() * pipSize * 5 * volatility)
    candles.push({ time, open: Number(open.toFixed(pricePrecision)), high: Number(high.toFixed(pricePrecision)), low: Number(low.toFixed(pricePrecision)), close: Number(close.toFixed(pricePrecision)), volume: Math.floor(100 + random() * 900) })
    price = close
  }
  return candles
}
...
export const validateCandles = (data: OHLCV[]): OHLCV[] => {
  const sorted = [...data].filter((c) => Number.isFinite(c.time) && Number.isFinite(c.open) && Number.isFinite(c.high) && Number.isFinite(c.low) && Number.isFinite(c.close)).filter((c) => c.high >= Math.max(c.open, c.close) && c.low <= Math.min(c.open, c.close)).sort((a, b) => a.time - b.time)
...
export const getMockCandles = (symbol: string, timeframe: Timeframe, limit = 300): OHLCV[] => {
  ...
  const raw = generateMockCandles({ count: Math.max(10, Math.min(limit, 1000)), startPrice: PRICE_BY_SYMBOL[symbol] ?? PRICE_BY_SYMBOL['EUR/USD'], pipSize, pricePrecision: specPrecision, timeframe, seed: hash(key) })
