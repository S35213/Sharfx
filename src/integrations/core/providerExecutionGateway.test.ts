...
describe('provider execution boundary', () => {
  it('allows external demo execution through the provider adapter', async () => {
    ...
    await expect(executeProviderOrder(adapter, { providerId: 'external', connectionId: 'c1', environment: 'demo', connectedAt: new Date().toISOString() }, 'acct', { symbol: 'EUR/USD', side: 'BUY', quantity: 1, quantityUnit: 'base', type: 'MARKET' })).resolves.toMatchObject({ providerOrderId: 'demo-1' })
  })
