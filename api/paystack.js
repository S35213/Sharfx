...
async function charge(req, res) {
  ...
  try {
    ...
    const botSlug = String(body.botSlug || '').trim(); const phone = normalizePhone(body.phone)
    ...
    const bot = (await catalogResponse.json())[0]; if (!bot) return json(res, 404, { ok: false, error: 'That bot is not currently available.' })
    const email = String(user.email || '').trim().toLowerCase(); if (!email) return json(res, 400, { ok: false, error: 'Your SHAFX account does not have an email address.' })
    const ref = reference()
    ...
    const paystackResponse = await fetch('https://api.paystack.co/charge', { method: 'POST', headers: { Authorization: `Bearer ${process.env.PAYSTACK_SECRET_KEY}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ email, amount: String(Math.round(Number(bot.price_kes) * 100)), currency: 'KES', reference: ref, mobile_money: { phone, provider: 'mpesa' }, metadata: { shafx_user_id: user.id, bot_slug: bot.slug, purchase_reference: ref, custom_fields: [{ display_name: 'SHAFX Bot', variable_name: 'shafx_bot', value: bot.name }, { display_name: 'SHAFX Purchase', variable_name: 'shafx_purchase', value: ref }] } }) })
    const data = await paystackResponse.json().catch(() => ({}))
    if (!paystackResponse.ok || !data.status) { await rest(`/shafx_bot_purchases?reference=eq.${encodeURIComponent(ref)}`, { method: 'PATCH', headers: { Prefer: 'return=minimal' }, body: JSON.stringify({ status: 'failed', provider_payload: data }) }).catch(() => {}); return json(res, 502, { ok: false, error: data.message || 'Paystack could not start the M-PESA payment.' }) }
...
async function status(req, res) {
  ...
  try {
    const user = await currentUser(req); if (!user) return json(res, 401, { ok: false, error: 'Not signed in' })
    const referenceValue = String(req.query.reference || '').trim(); if (!referenceValue) return json(res, 400, { ok: false, error: 'Missing purchase reference.' })
    const response = await rest(`/shafx_bot_purchases?user_id=eq.${encodeURIComponent(user.id)}&reference=eq.${encodeURIComponent(referenceValue)}&select=reference,bot_slug,status,amount_kes,paid_at,activated_at`)
...
async function webhook(req, res) {
  ...
  try {
    ...
    const payment = event.data || {}; const referenceValue = String(payment.reference || '')
    if (!referenceValue) return json(res, 400, { ok: false, error: 'Missing payment reference.' })
    const lookup = await rest(`/shafx_bot_purchases?reference=eq.${encodeURIComponent(referenceValue)}&select=id,user_id,bot_slug,amount_kes,status`)
    if (!lookup.ok) throw new Error('Unable to read SHAFX purchase.')
    const purchase = (await lookup.json())[0]; if (!purchase) return json(res, 404, { ok: false, error: 'Unknown SHAFX purchase reference.' })
    if (Number(payment.amount) !== Number(purchase.amount_kes) * 100 || String(payment.currency || '').toUpperCase() !== 'KES') return json(res, 400, { ok: false, error: 'Payment amount or currency does not match the SHAFX purchase.' })
    const rpcResponse = await rest('/rpc/activate_shafx_bot_purchase', { method: 'POST', body: JSON.stringify({ p_reference: referenceValue, p_provider_transaction_id: payment.id ? String(payment.id) : null, p_paid_at: payment.paid_at || null, p_payload: payment }) })
    if (!rpcResponse.ok) throw new Error('Unable to activate the SHAFX bot purchase.')
    const result = (await rpcResponse.json())[0]; if (!result?.ok) return json(res, 409, { ok: false, error: 'Payment received but the SHAFX bot could not be activated.' })
...
export default async function handler(req, res) {
  ...
  if (action === 'charge') return charge(req, res)
  if (action === 'status') return status(req, res)
  if (action === 'webhook') return webhook(req, res)
  return json(res, 400, { ok: false, error: 'Unknown Paystack action.' })
}